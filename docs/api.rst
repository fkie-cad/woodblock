...
Two optional constructor arguments let you control the overall layout of the
image. Passing :code:`scenario_gap=N` inserts :code:`N` blocks of padding between
consecutive scenarios, and :code:`target_size=N` pads the finished image up to
:code:`N` blocks in total. These mirror the :code:`scenario gap` and
:code:`image size` keys of the :code:`[general]` config section.
...
.. py:class:: woodblock.image.Image(block_size=512, padding_generator=None, scenario_gap=0, target_size=None)
   ...
   :param int scenario_gap: number of blocks of padding inserted between consecutive scenarios (default 0)
   :param int target_size: if set, pad the whole image up to this number of blocks
